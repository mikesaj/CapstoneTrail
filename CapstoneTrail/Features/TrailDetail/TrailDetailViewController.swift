...
class TrailDetailViewController: UIViewController, MKMapViewDelegate, UITextFieldDelegate {
    ...
    var groupId: String = ""
    //"22ED0638-C875-4B89-8D90-DE31BB5019AC"
    ...
    override func viewDidLoad() {
        ...
        coordinate2DList = TrailUtils.makeCoordinate2D(coordinates: coordinates)
    ...
    @IBAction func btnAdd_Click(_ sender: AnyObject) {
        ...
        let hikeScheduleReference = ref.child("hikingSchedules")
                                       .child(hikeId)
        hikeScheduleReference.child("groupId")
                             .setValue(self.groupId)
        hikeScheduleReference.child("trailId")
                             .setValue(self.id)
        hikeScheduleReference.child("trail")
                             .setValue(self.street)
        hikeScheduleReference.child("date")
                             .setValue(self.txtDateTrail.text)
        hikeScheduleReference.child("attendees")
                             .child("0")
                             .setValue(uid)
        // Add additional data for weather features
        hikeScheduleReference.child("area")
                             .setValue(self.area)
        if epochDate != nil {
            hikeScheduleReference.child("epochDate")
                                 .setValue(self.epochDate)
        }
